...
begin
  ...
  for x:=0 to BOARD_W-1 do
    for y:=0 to BOARD_H-1 do
      if Random<0.2 then // 20% bactérias
      begin
        Board[x,y].Tipo := tsBacteria;
        Board[x,y].Ent  := TBacteria.Create(1,1);
      end;
end;
...
begin
  ...
  if FCiclos = 200 then ColocaNovoSer(tsVegetariano, 1, 1);
  if FCiclos = 300 then ColocaNovoSer(tsCarnivoro, 1, 1);
